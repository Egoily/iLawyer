...
namespace ee.iLawyer.ViewModels
{
    ...
    //    public static Category ConvertToCategory(PropertyPick source) => new Category()
    //    {
    //        Id = source.Id,
    //        Code = source.Code,
    //        Name = source.Name,
    //        Icon = source.Icon,
    //    };
    //    public static PickerProperty ConvertToPickerProperty(PropertyPick source) => new PickerProperty()
    //    {
    ...
    //        try
    //        {
    //            var categories = MemoryCacheHelper.CacheItem<IList<PropertyPick>>(CacheKeys.PropertyItemCategories,
    //                delegate ()
    //                {
    //                    var server = new ILawyerServiceWebApi();
    //                    var response = server.GetPi(new GetPropertyPicksRequest());
    //                    if (response.Code == ErrorCodes.Ok && (response.QueryList?.Any() ?? false))
    //                    {
    //                        return response.QueryList.ToList();
    //                    }
    //                    return new List<PropertyPick>();
    //                },
